  ...
  `;
    ...
    &:hover { background: #0b51d9; box-shadow: 0 8px 18px rgba(16, 81, 217, 0.28); }
    &:active { transform: translateY(1px); }
    &:disabled { background: #98a2b3; box-shadow: none; cursor: not-allowed; }
    `;
  ...
  const scaleMeta = {
    cultural: {
      ...
      labels: ['1. 부정확하게 서술함', '2. 서술하지 않음', '3. 다소 서술함', '4. 대체로 서술함', '5. 매우 상세히 서술함'],
      bg: '#fffbdb',
    },
    visual: {
      title: '시각적 세부사항',
    ...
    hallucination: {
      ...
      labels: ['1. 전혀 없음', '2. 약간 있음', '3. 다소 있음', '4. 대부분 있음', '5. 완전히 있음'],
      bg: '#ffe6eb',
    },
  };
...
const Breadcrumbs = styled.nav`
  ...
  span.crumb {
    background: #f2f4f7; color: #475467; border: 1px solid #e5eaf0;
    border-radius: 8px; padding: 6px 10px; line-height: 1;
  }
  span.sep { color: #98a2b3; }
`;
  const PageInfo = styled.div` color: #667085; font-size: 14px; `;
  ...
  const LeftPanel = styled.div` display: grid; grid-template-rows: auto auto; gap: 16px; `;
  const ImageCard = styled.div` background: #fff; border: 1px solid #e5eaf0; border-radius: 12px; padding: 14px; `;
  const Image = styled.img` width: 100%; height: auto; max-height: 520px; object-fit: cover; border-radius: 10px; `;
  const CaptionCard = styled.div` background: #f9fafb; border: 1px solid #e5eaf0; border-radius: 12px; padding: 14px 16px; `;
  const CaptionHeader = styled.div` font-size: 13px; color: #667085; margin-bottom: 8px; `;
  const CaptionText = styled.div`
    background: #fff; border: 1px solid #e5eaf0; border-radius: 10px;
    padding: 14px 16px; color: #344054; line-height: 1.65;
  `;

  const RightPanel = styled.div` display: grid; gap: 16px; `;
  const Section = styled.section` border: 1px solid #e5eaf0; border-radius: 12px; padding: 16px 16px 12px; background: #fff; `;
  const SectionTitle = styled.h3` margin: 0 0 8px; font-size: 16px; color: #101828; `;
  const HelpText = styled.p` margin: 0 0 16px; color: #475467; font-size: 14px; line-height: 1.6; `;
  const SliderWrap = styled.div`
    padding: 8px 10px 12px; border-radius: 12px; background: ${(p) => p.bg || '#f5f5f5'};
  ...
  const ScaleLabels = styled.div`
    ...
    font-size: 11px; color: #667085; span { text-align: center; }
  `;
          ...
          {isSurveyComplete ? (
            ...
            <SurveyContainer>
              <SurveyTop>
                ...
                <PageInfo>
                  {Math.min(currentIndex + 1, captions.length || 0)} / {captions.length || 0}
                </PageInfo>
              </SurveyTop>
              ...
              <Body>
                <LeftPanel>
                  <ImageCard>
                    <Image src={image || BulgogiImg} alt={title || 'survey image'} />
                  </ImageCard>
                ...
                <RightPanel>
                  <Section>
                    ...
                    <SliderWrap bg={scaleMeta.cultural.bg}>
                      <SliderRail>
                        <Scale
                      ...
                      <ScaleLabels>
                        {scaleMeta.cultural.labels.map((l) => <span key={l}>{l}</span>)}
                      </ScaleLabels>
                    </SliderWrap>
                  </Section>
                  <Section>
                    ...
                    <SliderWrap bg={scaleMeta.visual.bg}>
                      <SliderRail>
                        <Scale
                      ...
                      <ScaleLabels>
                        {scaleMeta.visual.labels.map((l) => <span key={l}>{l}</span>)}
                      </ScaleLabels>
                    </SliderWrap>
                  </Section>
                  <Section>
                    ...
                    <SliderWrap bg={scaleMeta.hallucination.bg}>
                      <SliderRail>
                        <Scale
                      ...
                      <ScaleLabels>
                        {scaleMeta.hallucination.labels.map((l) => <span key={l}>{l}</span>)}
                      </ScaleLabels>
                    </SliderWrap>
                  </Section>
                </RightPanel>
