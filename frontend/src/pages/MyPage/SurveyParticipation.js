...
const SurveyParticipation = () => {
  ...
  const surveyMap = new Map(surveyArray.map((s) => [String(s?.surveyId), s]));
  ...
  const countryCharts = Object.entries(countryGroups).map(
    ([country, stats]) => ({
      country,
      data: [
            ...
            {participatedSurveys.length === 0 ? (
              ...
              <SurveyList>
                {participatedSurveys.map((s) => {
                  const matched = responseArray.find(
                    (r) => String(r?.surveyId) === String(s?.surveyId)
                  );
                  const answeredCount =
                    matched?.answers?.length ?? matched?.answerCount ?? 0;
